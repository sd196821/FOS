...
sdk_3_12B_data_t Pos;
sdk_3_12B_data_t Agl;
...
int16_t sdk_std_msgs_handler(uint8_t cmd_id,uint8_t* pbuf,uint16_t len,req_id_t req_id)
{
    ...
    Pos.x += recv_sdk_std_msgs.v.x;
    Pos.y += recv_sdk_std_msgs.v.y;
    Pos.z += recv_sdk_std_msgs.pos.height;
    float q0 = recv_sdk_std_msgs.q.q0;
    float q1 = recv_sdk_std_msgs.q.q1;
    float q2 = recv_sdk_std_msgs.q.q2;
    float q3 = recv_sdk_std_msgs.q.q3;
    Agl.x = atan2(2*(q0*q1+q2*q3),1-2*(q2*q2+q1*q1))*180/3.1415926;
    Agl.y = asin(2*(q0*q2-q1*q3))*180/3.1415926;
    Agl.z = atan2(2*(q0*q3+q2*q1),1-2*(q3*q3+q2*q2))*180/3.1415926;
...
void random_test_ack_cmd_callback(ProHeader *header)
{
	...
	*/
    ...
    memcpy((uint8_t *)&ack_data,((uint8_t *)header)+12, (header->length - EXC_DATA_SIZE));
...
void test_version_query_ack_cmd_callback(ProHeader *header)
{
	...
	printf("Sdk_ack_cmd0_callback,sequence_number=%d,session_id=%d,data_len=%d\n", header->sequence_number, header->session_id, header->length - EXC_DATA_SIZE);
    memcpy((uint8_t *)&ack,((uint8_t *)header)+12, sizeof(uint16_t));
	/* pay attention to memory holes */
    memcpy((uint8_t *)&ack.version_crc,((uint8_t *)header)+12, (header->length - EXC_DATA_SIZE)-sizeof(uint16_t));
